...
describe("Block", () => {
  ...
  //test that block contain all the elements that r required
  ...
  describe("adjust difficulty", () => {
    it("raises the difficulty for quickly mined block()", () => {
      expect(Block.adjustDifficulty({ originalBlock: block, timestamp: block.timestamp + MINE_RATE - 100 })).toEqual(
        block.difficulty + 1
      );
    });
    it("lowers the difficulty for slwoy mined block()", () => {
      expect(Block.adjustDifficulty({ originalBlock: block, timestamp: block.timestamp + MINE_RATE + 100 })).toEqual(
        block.difficulty - 1
      );
    });
